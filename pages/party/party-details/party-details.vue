	...
	<view v-if="!loading">
		<view class="bg-white margin-bottom-lg">
			<cu-custom bgColor="kpbg-black" class="text-white" :isBack="true">
				...
				<block slot="content" class="text-white">派对详情</block>
			</cu-custom>
			...
			<view style="height: 474rpx;">
				<image :src="epartyDetail.bannerImgpath" mode='scaleToFill' style="width: 100%; height: 100%;"></image>
			</view>
			...
			<view>
				...
				<view class="margin  padding-left" style="border-left: 8rpx solid #FBD660;">
					<view>
						<text>{{epartyDetail.startTime}}</text>
						<text class="margin-xs">至</text>
						<text class="margin-xs">{{epartyDetail.endTime}}</text>
					</view>
					<view class="margin-tb-lg">
						<text>免费</text> <text>已报 <text>{{epartyDetail.joinedMember}}/{{epartyDetail.maxMember}}</text>人</text>
					</view>
					<view>
			...
			<view class="margin">
				...
				<view>{{epartyDetail.content}}</view>
			</view>
			<view class="margin-left margin-top bg-white">
				<view class="margin-top-xl margin-bottom-xs text-xl text-black" style="font-weight: 550;">
					<view>已报名用户<text class="margin-left-xs">（{{epartyDetail.joinedMember}}/{{epartyDetail.maxMember}}）</text></view>
				</view>
			</view>
			<view class="padding-sm padding-bottom-xl">
				<view>
					<view class="cu-avatar round lg margin-xs" v-for="(item,index) in 1" :key="index" :style="[{ backgroundImage:'url(' +item+ ')' }]"></view>
				</view>
			</view>
		</view>
		...
		<tui-bottom-popup :show="popupShow&&isRead" @close="hidePopup">
			<view class="tui-popup-box">
				...
				<view class=" text-center">
					<button class="cu-btn padding-lr-lg round bg-orange kp-btn shadow-blur" @tap="pid==0?'':NavigateToOderSubmit(epid,orderType,count)">购买</button>
				</view>
			</view>
		</tui-bottom-popup>
	</view>
...
<script>
	...
	export default {
		...
		async mounted() {
			const epartyDetailRes = await getEpartyDetail(this.eid);
			this.epartyDetail = epartyDetailRes.data.eparty;

			// this.Store = this.epartyDetail.store;
			console.log(this.epartyDetail)
...
<!-- <template>
	<view>
		<view class="bg-white" >
			...
			<view>
				<image src="https://ss0.bdstatic.com/70cFvHSh_Q1YnxGkpoWK1HF6hhy/it/u=2548273986,3713164240&fm=11&gp=0.jpg" style="width: 100%;height: 450rpx;"></image>
			</view>
			<view>
				<view class="margin margin-top-xl text-xxl text-black" style="font-weight: 550;">活动名称名</view>
				<view class="margin  padding-left" style="border-left: 8rpx solid #FBD660;">
					<view>
						<text>2018年10月06日</text><text class="margin-xs">星期六</text><text class="margin-xs">08：00~ 12：00</text>
					</view>
					<view class="margin-tb-lg">
						<text>免费</text> <text>已报 <text>10/12</text>人</text>
					</view>
					<view>
						<view style="width: 70%; display: inline-block;">广东省广州市白云区广园中路白云山南路白云山风景名胜区西门</view>
							<view class="fr margin-top margin-left-xs" style="width: 30rpx;height: 10rpx;border: 1rpx solid #7A7A7A; border-radius: 50%;">
								<view class="cuIcon-location text-black" style="margin-top: -22rpx; "></view>
							</view>
					</view>
			...
			<view class="margin">
				<view class="margin-top-xl margin-bottom-xs text-xl text-black" style="font-weight: 550;">活动说明</view>
				<view >本活动有某某组织，有某某组织，有某某组织，有某某组织，有某某组织，有某某组织，有某某组织，有某某组织，有某某组织</view>			
			</view>
			<view class="margin-left margin-top bg-white">
				<view class="margin-top-xl margin-bottom-xs text-xl text-black" style="font-weight: 550;">
					<view>已报名用户<text class="margin-left-xs">（10/12）</text></view>
				</view>
			</view>
			<view class="padding-sm">
				<view>
					<view class="cu-avatar round lg margin-xs"  v-for="(item,index) in avatar" :key="index" :style="[{ backgroundImage:'url(' + avatar[index] + ')' }]"></view>
				</view>
			</view>
		</view>
		<tui-divider class="">这里已经是底部啦</tui-divider>
	</view>
