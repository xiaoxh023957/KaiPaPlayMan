	...
	<view class="bg-white" v-if='!loading'>
		...
		<view class="cu-bar search kpbg-black">
			...
			<view class="search-form round ">
				...
				<input @focus="InputFocus" @blur="InputBlur" :adjust-position="false" type="text" placeholder="请输入店铺名称" @click="jump"
			...
			<view class="flex">
				<view class="flex-twice margin-bottom-xs margin-right-xs" style="border-radius: 30rpx 0 0 0;overflow: hidden;"
				 @click="mysteriousStoreGameList()">
					<image style="width:100%;height:200rpx;" src="https://ss1.bdstatic.com/70cFvXSh_Q1YnxGkpoWK1HF6hhy/it/u=2960677927,138788196&fm=26&gp=0.jpg"></image>
				</view>
		...
		<!-- 派对入口 -->
		<view class="bg-white padding " @click="party()">
			<view class="padding kp-userContRadius" style=" height:160rpx; background-image: url(https://ss0.bdstatic.com/70cFvHSh_Q1YnxGkpoWK1HF6hhy/it/u=1017486331,1394773415&fm=26&gp=0.jpg); background-size:100%;">
			</view>
		</view>
		
		<!-- 限时抢购 -->
		<view class="bg-white padding " @click="limit">
			<view class="padding kp-userContRadius" style=" height:160rpx; background-image: url(https://ss0.bdstatic.com/70cFvHSh_Q1YnxGkpoWK1HF6hhy/it/u=1017486331,1394773415&fm=26&gp=0.jpg); background-size:100%;">
			</view>
		</view>

		<!-- 一百购物 mall -->
		<view class="bg-white padding " @click="mall">
			<view class="padding kp-userContRadius" style=" height:160rpx; background-image: url(https://ss0.bdstatic.com/70cFvHSh_Q1YnxGkpoWK1HF6hhy/it/u=1017486331,1394773415&fm=26&gp=0.jpg); background-size:100%;">
			</view>
		</view>
		...
		<view v-else @click="NavgetaToMoreStoreList(stypename,stypeid,currPage)">
			<tui-divider width="80%">点击查看更多</tui-divider>
		</view>
...
<script>
	...
	export default {
		...
		mounted: async function(options) {
			...
			//获取商品分类列表
			const getGoodsTypeListRes = await getGoodsTypeList({pageindex:1,pagesize:999,type:1});
