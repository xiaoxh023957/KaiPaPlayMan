		...
		<view v-if="!loading" class="container">
			...
			<view class="header">
				<view class="header-title">
					<view class="left">
						开趴币： <text style="color: #E4431A;">{{userInfo.gold}}</text>
					</view>
	...
	export default {
		...
		methods: {
			...
			initdata: async function(that) {
				...
				const turntableInfo = await getTurnTableData();
				this.turntableData = turntableInfo.data.Turntable;
			...
			playReward: async function() {
				...
				const deductData = await deductGold(1, that.uid);
				if (deductData.data.code == 500) {
					...
					uni.setStorageSync("USERDATA", deductData.data.userInfo)
					// console.log(res)
						...
						setTimeout(function() {
							uni.showModal({
								...
								success: function() {
									...
									//将winningInfo的ctypeid获取，和当前用户ID一起提交给后台发奖
